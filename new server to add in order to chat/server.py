...
class Server:
    ...
    def checkForm(self, msg):
        ...
        if (a[0] != "SEND" or b[0] != "Content-length:" or recipient == '' or content == '' or len(content) != int(
                length)):  # if invalid form appears.
            form = False
    ...
    def unicast(self, recipient, username, content, clientSoc):
        forwardSoc = map[
            recipient]  # getting the socket from the map entry where the message to be forwarded (ie. receiving socket registered by client).
        forwardMsg = "FORWARD {}\nContent-length: {}\n\n{}".format(username, len(content),
                                                                   content)  # creating forward message in format: "FORWARD [username] Content-length: [length of message] [message content]"
        ...
        if (arr[0] == "RECEIVED" and arr[1] == "{}\n\n".format(username)):  # checking the message format.
            print('Message forwarded from {} to {}'.format(username,
                                                           recipient))  # username is sender and recipient is receiver.
            ack_msg = "SEND {}\n\n".format(recipient)
    ...
    def registerAndForward(self, clientSoc):
        ...
        if (a[0] == 'REGISTER' and a[
            1] == 'TOSEND'):  # checking the message according to format: "REGISTER TOSEND [username]".
            if (a[2].isalnum()):
                ...
                ack = 'REGISTERED TOSEND {}\n\n'.format(
                    username)  # creating the response messsage to client in format: "REGISTERED TOSEND [username]".
            else:
                print("The username should be Alphanumeric")

        elif (a[0] == 'REGISTER' and a[
            1] == 'TORECV'):  # checking the message according to format: "REGISTER TORECV [username]".
            if (a[2].isalnum()):
                ...
                ack = 'REGISTERED TORECV {}\n\n'.format(
                    username)  # creating the response message to client in format: "REGISTERED TORECV [username]".
            else:
                print("The Username should be Alphanumeric")
            ...
            print(
                'ERROR 101 : No user registered. Please register this user first.')  # server response to client for sny communication before registration.
        ...
        # server will receive messages and forward them
        while True:
            try:
            ...
            if (form == False):  # if message found in invalid form
                ...
                if recipient in map.keys():  # if client is in map means client is already registered.
                    self.unicast(recipient, username, content,
                                 clientSoc)  # calling unicast function for single recipient
                else:
                    print("ERROR 102 : Unable to Send")
                    print("Message not delivered from {} to {}".format(username, recipient))
...
while True:
    ...
    thread = threading.Thread(target=svr.registerAndForward,
                              args=(clientSoc,))  # creating multithreads for multiple connections at same port
